...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        int num = in.nextInt(); //Milo's commment: previously a compile time error as it was not properly written out

        // calculate the cost before discount.
        double cost = num * COST_OF_COOKIE;

        // Use conditional statements to determine what discount is.
        double discount = 0.0; //Milo's comment: previously a compile time error, discount needed a value when number of cookies is less than or equal to 6.  
        if (num > 6 && num <= 12) { //Milo's comment: logic error: needed to be greater than 6 AND less than or equal to 12
            discount = 0.025;
        } else if (num > 12 && num <= 24) { //Milo's comment: logic error: needed to be greater than 12 AND less than or equal to 24
            discount = 0.05;
        } else if (num > 24) {
            discount = 0.15;
        }
